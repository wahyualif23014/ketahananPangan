...
<div class="space-y-8 pb-24 kelola-container max-w-[1600px] mx-auto" x-data="kelolaLahan()">
        ...
        <div class="overflow-x-auto custom-scrollbar p-6">
            <table class="w-full text-left border-collapse min-w-[900px]">
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    @php
                        $isPolresTable = substr_count(auth()->user()->id_tugas ?? '', '.') < 2;
                        $allActiveTanams = collect($row->history_tanam)->filter(fn($t) => ($t->is_active ?? 1) == 1);
                        
                        $activePanens = collect();
                        foreach ($allActiveTanams as $t) {
                            if (isset($t->panens)) {
                                $activePanens = $activePanens->merge($t->panens);
                            }
                        }
                        
                        $activeDistribusis = collect();
                        foreach ($activePanens as $p) {
                            if (isset($p->distribusis)) {
                                $activeDistribusis = $activeDistribusis->merge($p->distribusis);
                            }
                        }
                    @endphp
                    <tr x-show="isResorOpen('{{ $resorId }}')"
                        ...
                        <td class="px-4 py-6 border-x border-slate-50 align-top h-full">
                            ...
                            <div class="flex flex-col justify-between h-full min-h-[150px]">
                                <div class="flex flex-col gap-3">
                                        ...
                                        @php
                                            $isPolresTable = substr_count(auth()->user()->id_tugas ?? '', '.') < 2;
                                            $totalTanamHa = collect($row->history_tanam)->filter(fn($t) => ($t->is_active ?? 1) == 1)->sum('luas_tanam');
                                        @endphp
                                ...
                                <div class="flex flex-col justify-end gap-2 mt-3">
                                    @php
                                        $unvalidatedTanams = $isPolresTable ? $allActiveTanams->filter(fn($t) => is_null($t->valid_oleh) && empty($t->alasan_tolak)) : collect();
                                        $rejectedTanams = $isPolresTable ? $allActiveTanams->filter(fn($t) => !empty($t->alasan_tolak)) : collect();
                                        $allValidated = $isPolresTable ? ($allActiveTanams->isNotEmpty() && $allActiveTanams->every(fn($t) => !is_null($t->valid_oleh))) : ($row->tanam_valid_oleh);
                                    @endphp

                                    @if($isPolresTable && $unvalidatedTanams->isNotEmpty())
                                    <div class="flex flex-col gap-1 w-full">
                                        <button @click="submitValidasiDirect('{{ $unvalidatedTanams->pluck('id_tanam')->implode(',') }}', 'tanam')" type="button" class="px-2 py-1 bg-emerald-50 border border-emerald-100 text-emerald-600 rounded text-[9px] font-black uppercase hover:bg-emerald-500 hover:text-white transition-colors shadow-sm w-full text-center">Validasi</button>
                                        <button @click="submitTolakDirect('{{ $unvalidatedTanams->pluck('id_tanam')->implode(',') }}', 'tanam', '{{ addslashes($row->nama_wilayah ?? $row->alamat_lahan ?? '') }}')" type="button" class="px-2 py-1 bg-rose-50 border border-rose-100 text-rose-600 rounded text-[9px] font-black uppercase hover:bg-rose-500 hover:text-white transition-colors shadow-sm w-full text-center">Tolak</button>
                                    </div>
                                    @elseif($isPolresTable && $rejectedTanams->isNotEmpty())
                                    <div class="flex flex-col">
                                        <span class="inline-flex items-center w-fit gap-1 text-[9px] font-black text-rose-600 bg-rose-50 border border-rose-200 px-2 py-1 rounded-lg shadow-sm">
                                        ...
                                        <div class="text-[8px] text-rose-400 mt-1 italic line-clamp-2" title="{{ $rejectedTanams->first()->alasan_tolak }}">{{ $rejectedTanams->first()->alasan_tolak }}</div>
                                    </div>
                                    @elseif($allValidated)
                                    ...
                                    @elseif(!$isPolresTable && !$row->tanam_valid_oleh && empty($row->tanam_alasan_tolak))
                                    ...
                                    @elseif(!$isPolresTable && !empty($row->tanam_alasan_tolak))
                                    <div class="flex flex-col">
                                        <span class="inline-flex items-center w-fit gap-1 text-[9px] font-black text-rose-600 bg-rose-50 border border-rose-200 px-2 py-1 rounded-lg shadow-sm">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg> DITOLAK
                                        </span>
                                        <div class="text-[8px] text-rose-400 mt-1 italic line-clamp-2" title="{{ $row->tanam_alasan_tolak }}">{{ $row->tanam_alasan_tolak }}</div>
                                    </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        <td class="px-4 py-6 border-r border-slate-50 align-top h-full">
                            ...
                            <div class="flex flex-col justify-between h-full min-h-[150px]">
                                ...
                                <div class="flex flex-col justify-end gap-2 mt-3">
                                    @php
                                        $unvalidatedPanens = $isPolresTable ? $activePanens->filter(fn($p) => is_null($p->valid_oleh) && empty($p->alasan_tolak)) : collect();
                                    @endphp
                                    @if($isPolresTable && $unvalidatedPanens->isNotEmpty())
                                    <div class="flex items-center gap-1.5 w-full">
                                        <form action="{{ route($routePrefix.'.kelola-lahan.panen.validasi', $unvalidatedPanens->pluck('id_panen')->implode(',')) }}" method="POST" data-ajax="true" class="m-0 flex-1">
                                            @csrf @method('PUT')
                                            <button class="w-full px-2 py-1.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-md text-[9px] font-black uppercase hover:bg-amber-500 hover:text-white transition-colors shadow-sm">Validasi</button>
                                        </form>
                                        <button @click="submitTolakDirect('{{ $unvalidatedPanens->pluck('id_panen')->implode(',') }}', 'panen', '{{ addslashes($row->nama_wilayah ?? $row->alamat_lahan ?? '') }}')" type="button" class="flex-1 px-2 py-1.5 bg-rose-50 border border-rose-200 text-rose-700 rounded-md text-[9px] font-black uppercase hover:bg-rose-500 hover:text-white transition-colors shadow-sm">Tolak</button>
                                    </div>
                                    @elseif($allPanenValidated)
                                    <div class="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-50 border border-emerald-200 rounded-md shadow-sm w-fit">
                                        <svg class="w-3.5 h-3.5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                                        <span class="text-[9px] font-black text-emerald-700 uppercase tracking-wider">TERVALIDASI</span>
                                    </div>
                        ...
                        <td class="px-4 py-6 border-r border-slate-50 align-top h-full">
                            ...
                            <div class="flex flex-col justify-between h-full min-h-[150px]">
                                ...
                                <div class="flex flex-col justify-end gap-2 mt-3">
                                    ...
                                    @elseif($isPolres)
                                    @php
                                        $pendingDistribusis = $activeDistribusis->filter(fn($d) => !$d->valid_oleh && !$d->alasan_tolak);
                                        $pendingIds = $pendingDistribusis->pluck('id_distribusi')->implode(',');
                                    @endphp
                                    @if($pendingDistribusis->isNotEmpty())
                                    <div class="flex items-center gap-1.5 w-full">
                                        <form action="{{ route($routePrefix.'.kelola-lahan.serapan.validasi', $pendingIds) }}" method="POST" data-ajax="true" class="m-0 flex-1">
                                            @csrf @method('PUT')
                                            <button class="w-full px-2 py-1.5 bg-blue-50 border border-blue-200 text-blue-700 rounded-md text-[9px] font-black uppercase hover:bg-blue-500 hover:text-white transition-colors shadow-sm">Validasi</button>
                                        </form>
                                        <button @click="submitTolakDirect('{{ $pendingIds }}', 'serapan', '{{ addslashes($row->nama_wilayah ?? $row->alamat_lahan ?? '') }}')" type="button" class="flex-1 px-2 py-1.5 bg-rose-50 border border-rose-200 text-rose-700 rounded-md text-[9px] font-black uppercase hover:bg-rose-500 hover:text-white transition-colors shadow-sm">Tolak</button>
                                    </div>
                                    @endif
                                    @else
                                    <div class="flex items-center gap-1.5 px-2.5 py-1.5 bg-amber-50 border border-amber-200 rounded-md shadow-sm w-fit">
                        ...
                        <td class="px-4 py-6 text-right align-top">
                            <div class="flex flex-col items-end gap-2">
                                @php
                                $mainTanam = $row->history_tanam->where('is_active', 1)->first();
                                $canSelesaiMain = false;
                                if($mainTanam) {
                                    $allSerapans = $mainTanam->panens->flatMap(fn($p) => $p->distribusis ?? collect());
                                    $canSelesaiMain = !is_null($mainTanam->valid_oleh) &&
                                        $mainTanam->panens->count() > 0 &&
                                        $mainTanam->panens->every(fn($p) => !is_null($p->valid_oleh)) &&
                                        $allSerapans->count() > 0 &&
                                        $allSerapans->every(fn($s) => !is_null($s->valid_oleh));
                                }
                                @endphp

                                @if($mainTanam && auth()->user()->role === 'admin')
                                    @if($mainTanam->status_akhiri_siklus == 1 || $canSelesaiMain)
                                        <button @click="terimaAkhiriSiklus('{{ $mainTanam->id_tanam }}')" title="Selesai Siklus" class="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase hover:bg-indigo-700 transition-all shadow-sm flex items-center gap-1">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Detail and Edit Lahan Buttons Removed -->
                        ...
                        <td colspan="6" class="px-6 pb-6 bg-slate-50/40">
                            <div class="rounded-[2rem] border border-slate-200 bg-white shadow-lg shadow-slate-200/30 p-6">
                                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 pb-5 border-b border-slate-100">
                                    ...
                                    @php
                                        $sisaLahanComputed = max(0, $row->luas_lahan - $row->history_tanam->filter(fn($t) => ($t->is_active ?? 1) == 1)->sum('luas_tanam'));
                                    @endphp
                                    @if(in_array(auth()->user()->role, ['admin', 'operator']) && $sisaLahanComputed > 0)
                                    <button @click='openStageModal("{{ $row->id_lahan }}", @json(collect($row)->merge(['sisa_lahan' => $sisaLahanComputed])), 0)' class="px-4 py-2 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase hover:bg-emerald-600 transition-all shadow-md shadow-emerald-500/20 flex items-center gap-2">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"></path>
                                        </svg>
                                        Tambah Tanam Baru
                                        ...
                                        <div class="bg-slate-50/50 rounded-xl border border-slate-100 p-5 hover:border-emerald-200 transition-colors">
                                            <div class="flex justify-between items-start mb-4 pb-4 border-b border-slate-100">
                                                ...
                                                <div class="flex gap-2">
                                                    @php
                                                    $allSerapans = $tanam->panens->flatMap(fn($p) => $p->distribusis ?? collect());
                                                    ...
                                                    $allSerapans->every(fn($s) => !is_null($s->valid_oleh));
                                                    @endphp
                                                    @if($tanam->status_akhiri_siklus == 1)
                                                         @if(auth()->user()->role === 'admin')
                                                         <div class="flex flex-col gap-1 w-full max-w-[150px]">
                                                             <button @click="terimaAkhiriSiklus('{{ $tanam->id_tanam }}')" type="button" class="px-2.5 py-1.5 bg-indigo-600 text-white rounded-lg text-[9px] font-black uppercase hover:bg-indigo-700 transition-all shadow-sm flex justify-center items-center gap-1 w-full"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path>
                                                     ...
                                                     @else
                                                         @if($canSelesai)
                                                             @if(auth()->user()->role === 'admin')
                                                             <button @click="terimaAkhiriSiklus('{{ $tanam->id_tanam }}')" type="button" class="px-2.5 py-1.5 bg-indigo-600 text-white rounded-lg text-[9px] font-black uppercase hover:bg-indigo-700 transition-all shadow-sm flex items-center gap-1 w-fit"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path>
                                                                 </svg> Selesai Siklus</button>
                                                             @elseif($isPolres)
                                                             <button @click="ajukanAkhiriSiklus('{{ $tanam->id_tanam }}')" type="button" class="px-2.5 py-1.5 bg-indigo-600 text-white rounded-lg text-[9px] font-black uppercase hover:bg-indigo-700 transition-all shadow-sm flex items-center gap-1 w-fit"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                                 </svg> Ajukan Akhiri Siklus</button>
                                                             @endif
                                                         @endif
                                                     @endif
                                                    @if($tanam->alasan_tolak_akhiri_siklus && $tanam->status_akhiri_siklus == 0)
                                                    <div class="flex flex-col">
                                                        <span class="inline-flex items-center w-fit gap-1 text-[9px] font-black text-rose-600 bg-rose-50 border border-rose-200 px-2 py-1 rounded-lg shadow-sm">❌ AKHIR SIKLUS DITOLAK</span>
                                                        <div class="text-[8px] text-rose-400 mt-1 italic max-w-[150px]">{{ $tanam->alasan_tolak_akhiri_siklus }}</div>
                                                    </div>
                                                    @endif
                                                    {{-- Tolak Siklus dihapus --}}
                                                    ...
                                                    <div class="flex flex-col">
                                                        ...
                                                        <div class="text-[8px] text-rose-400 mt-1 italic line-clamp-2" title="{{ $tanam->alasan_tolak }}">{{ $tanam->alasan_tolak }}</div>
                                                    </div>
                                                    @else
                                                    @if($isPolres)
                                                    <div class="flex items-center gap-1">
                                                        <button @click="submitValidasiDirect('{{ $tanam->id_tanam }}', 'tanam')" type="button" class="px-2.5 py-1.5 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-lg text-[9px] font-black uppercase hover:bg-emerald-500 hover:text-white transition-all shadow-sm">Validasi</button>
                                                        <button @click="submitTolakDirect('{{ $tanam->id_tanam }}', 'tanam')" type="button" class="px-2.5 py-1.5 bg-rose-50 text-rose-600 border border-rose-200 rounded-lg text-[9px] font-black uppercase hover:bg-rose-500 hover:text-white transition-all shadow-sm">Tolak</button>
                                                    </div>
                                                    @else
                                                    ...
                                                    @else
                                                    @if($isPolres)
                                                    <form action="{{ route($routePrefix.'.kelola-lahan.tanam.unvalidasi', $tanam->id_tanam) }}" method="POST" data-ajax="true" class="m-0">
                                                    @csrf @method('PUT')
                                                    <button type="submit" title="Batalkan Validasi" class="px-2.5 py-1.5 bg-emerald-500 text-white rounded-lg text-[9px] font-black uppercase hover:bg-emerald-600 transition-colors">
                                                        TERVALIDASI
                                                    </button>
                                                    </form>
                                                    @else
                                                    <span class="inline-flex items-center gap-1 text-[9px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-1.5 rounded-lg shadow-sm">
                                                        Sudah Tervalidasi
                                                    </span>
                                                    @endif
                                                    @endif
                                                    @if(auth()->user()->role === 'admin' || (auth()->user()->role === 'operator' && substr_count((string)auth()->user()->id_tugas, '.') >= 2 && $tanam->alasan_tolak))
                                                    ...
                                                    @php
                                                        $sisaPanenComputed = max(0, $tanam->luas_tanam - $tanam->panens->sum('luas_panen'));
                                                    @endphp
                                                    @if(!is_null($tanam->valid_oleh) && $sisaPanenComputed > 0)
                                                    @php
                                                        $panenData = collect($row)->merge(["sisa_panen_ha" => $sisaPanenComputed]);
                                                    @endphp
                                                    <button @click='openStageModal("{{ $row->id_lahan }}", @json($panenData), 1, "{{ $tanam->id_tanam }}")' class="px-3 py-1.5 bg-amber-500 text-white rounded-lg text-[9px] font-black uppercase tracking-wider hover:bg-amber-600 transition-all shadow-sm shadow-amber-500/20 ml-2">+ Panen</button>
                                                    @endif
                                                    @endif
                                                </div>
                                            </div>
                                                        ...
                                                        <div class="p-4 bg-amber-50/30 flex justify-between items-start border-b border-amber-100">
                                                            ...
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <div class="flex flex-col">
                                                                    ...
                                                                    <div class="text-[8px] text-rose-400 mt-1 italic line-clamp-2" title="{{ $panen->alasan_tolak }}">{{ $panen->alasan_tolak }}</div>
                                                                </div>
                                                                @else
                                                                @if($isPolres)
                                                                <button @click="submitValidasiDirect('{{ $panen->id_panen }}', 'panen')" type="button" class="px-2 py-1 bg-amber-50 border border-amber-100 text-amber-600 rounded text-[9px] font-black uppercase hover:bg-amber-500 hover:text-white transition-colors">Validasi</button>
                                                                ...
                                                                @else
                                                                @if($isPolres)
                                                                <form action="{{ route($routePrefix.'.kelola-lahan.panen.unvalidasi', $panen->id_panen) }}" method="POST" data-ajax="true" class="m-0">
                                                                @csrf @method('PUT')
                                                                <button type="submit" title="Batalkan Validasi" class="px-2 py-1 bg-emerald-500 text-white rounded text-[9px] font-black uppercase hover:bg-emerald-600 transition-colors">
                                                                    TERVALIDASI
                                                                </button>
                                                                </form>
                                                                @else
                                                                ...
                                                                @endif
                                                                @endif
                                                                @if(auth()->user()->role === 'admin' || (!$isPolres && $panen->alasan_tolak))
                                                                @php
                                                                    $panenSisaEditHa = max(0, $tanam->luas_tanam - $tanam->panens->where('id_panen', '!=', $panen->id_panen)->sum('luas_panen'));
                                                                    $editPanenData = array_merge((array)$row, (array)$tanam, (array)$panen, ["sisa_panen_ha" => $panenSisaEditHa]);
                                                                @endphp
                                                                <button @click='editPanen("{{ $panen->id_panen }}", @json($editPanenData))' class="px-2 py-1 bg-white text-amber-600 border border-amber-200 rounded text-[9px] font-black uppercase hover:bg-amber-50 transition-colors">Edit</button>
                                                                @endif
                                                                @if(auth()->user()->role === 'admin')
                                                                ...
                                                                @if(in_array(auth()->user()->role, ['admin', 'operator']))
                                                                @php
                                                                    $tanamSisa = max(0, $row->luas_lahan - $row->history_tanam->filter(fn($t) => ($t->is_active ?? 1) == 1)->sum('luas_tanam'));
                                                                    $panenSisa = max(0, $panen->total_panen - $panen->distribusis->sum('total_distribusi'));
                                                                    $serapanData = collect($row)->merge(['sisa_lahan' => $tanamSisa, 'sisa_panen' => $panenSisa, 'tgl_panen' => $panen->tgl_panen]);
                                                                @endphp
                                                                @if(!is_null($panen->valid_oleh) && $panenSisa > 0)
                                                                <button @click='openStageModal("{{ $row->id_lahan }}", @json($serapanData), 2, "{{ $tanam->id_tanam }}", "{{ $panen->id_panen }}")' class="px-3 py-1.5 bg-blue-500 text-white rounded-lg text-[9px] font-black uppercase hover:bg-blue-600 shadow-sm ml-2">+ Serapan</button>
                                                                @endif
                                                                @endif
                                                            </div>
                                                        </div>
                                                                ...
                                                                <div class="bg-slate-50 rounded-lg border border-slate-100 p-3 flex justify-between items-center">
                                                                    ...
                                                                    <div class="flex items-center gap-2">
                                                                        ...
                                                                        <div class="flex flex-col">
                                                                            ...
                                                                            <div class="text-[8px] text-rose-400 mt-1 italic line-clamp-2" title="{{ $distribusi->alasan_tolak }}">{{ $distribusi->alasan_tolak }}</div>
                                                                        </div>
                                                                        @else
                                                                        @if($isPolres)
                                                                        <button @click="submitValidasiDirect('{{ $distribusi->id_distribusi }}', 'serapan')" type="button" class="px-2 py-1 bg-blue-50 border border-blue-100 text-blue-600 rounded text-[8px] font-black uppercase hover:bg-blue-500 hover:text-white transition-colors">Validasi</button>
                                                                        ...
                                                                        @else
                                                                        @if($isPolres)
                                                                        <form action="{{ route($routePrefix.'.kelola-lahan.serapan.unvalidasi', $distribusi->id_distribusi) }}" method="POST" data-ajax="true" class="m-0">
                                                                        @csrf @method('PUT')
                                                                        <button type="submit" title="Batalkan Validasi" class="px-2 py-1 bg-emerald-500 text-white rounded text-[8px] font-black uppercase hover:bg-emerald-600 transition-colors">
                                                                            TERVALIDASI
                                                                        </button>
                                                                        </form>
                                                                        @else
                                                                        ...
                                                                        @endif
                                                                        @endif
                                                                        @if(auth()->user()->role === 'admin' || (auth()->user()->role === 'operator' && substr_count((string)auth()->user()->id_tugas, '.') >= 2 && $distribusi->alasan_tolak))
                                                                        @php
                                                                            $panenSisaEdit = max(0, $panen->total_panen - $panen->distribusis->where('id_distribusi', '!=', $distribusi->id_distribusi)->sum('total_distribusi'));
                                                                            $editSerapanData = array_merge((array)$row, (array)$tanam, (array)$panen, (array)$distribusi, ['sisa_panen' => $panenSisaEdit]);
                                                                        @endphp
                                                                        <button @click='editSerapan("{{ $distribusi->id_distribusi }}", @json($editSerapanData))' class="px-2 py-1 bg-white text-blue-600 border border-blue-200 rounded text-[8px] font-black uppercase hover:bg-blue-50 transition-colors">Edit</button>
                                                                        @endif
                                                                        @if(auth()->user()->role === 'admin')
    ...
    <script>
        function kelolaLahan() {
            return {
                ...
                checkLuasTanamRealtime(value) {
                    const inputLuas = parseFloat(value || 0);
                    const sisa = this.sisaLahan;
                    if (sisa <= 0 && !this.isEditMode) {
                        this.notify('error', 'Kapasitas Lahan Penuh', 'Sisa kapasitas lahan sudah habis.');
                        this.formTanam.luas_tanam = '';
                        return;
                    }
                    if (inputLuas > sisa) {
                        this.notify('warning', 'Melebihi Kapasitas', `Luas tanam (${inputLuas} Ha) melebihi sisa kapasitas yang tersedia (${sisa.toFixed(2)} Ha).`);
                    }
                },

                checkLuasPanenRealtime(value) {
                    const inputLuas = parseFloat(value || 0);
                    let targetTanam = null;
                    if (this.activeLahan?.history_tanam) {
                        const historyArray = Array.isArray(this.activeLahan.history_tanam) ? this.activeLahan.history_tanam : Object.values(this.activeLahan.history_tanam);
                    ...
                    const sisa = parseFloat(this.activeLahan?.sisa_panen_ha ?? targetTanam?.luas_tanam ?? this.activeLahan?.luas_tanam ?? 0);
                    if (sisa <= 0 && !this.isEditMode) {
                        this.notify('error', 'Kapasitas Lahan Penuh', 'Sisa luas tanam yang bisa dipanen sudah habis.');
                        this.formPanen.luas_panen = '';
                        return;
                    }
                    if (Number(this.formPanen.status_panen) !== 2 && inputLuas > sisa) {
                        this.notify('warning', 'Melebihi Kapasitas', `Luas panen (${inputLuas} Ha) melebihi sisa luas tanam yang tersedia (${sisa.toFixed(2)} Ha).`);
                    }
                },

                checkSerapanRealtime(value) {
                    const inputLuas = parseFloat(value || 0);
                    const sisa = this.sisaPanen;
                    if (sisa <= 0 && !this.isEditMode) {
                        this.notify('error', 'Kapasitas Serapan Habis', 'Sisa hasil panen untuk didistribusikan sudah habis.');
                        this.formSerapan.total_distribusi = '';
                        return;
                    }
                    if (inputLuas > sisa) {
                        this.notify('warning', 'Melebihi Kapasitas', `Total Serapan (${inputLuas} Ton) melebihi sisa hasil panen yang tersedia (${sisa.toFixed(2)} Ton).`);
                    }
                },

                get activeTanamData() {
                    if (!this.activeLahan) return null;
                    if (this.activeLahan.history_tanam) {
                        const historyArray = Array.isArray(this.activeLahan.history_tanam) ? this.activeLahan.history_tanam : Object.values(this.activeLahan.history_tanam);
                        return historyArray.find(t => String(t.id_tanam) === String(this.activeTanamId ?? this.activeLahan.id_tanam)) || this.activeLahan;
                    }
                    return this.activeLahan;
                },

                get sisaPanen() {
                    return parseFloat(this.activeLahan?.sisa_panen ?? 0);
                },

                checkSerapanRealtime(value) {
                    const inputSerapan = parseFloat(value || 0);
                    const sisa = this.sisaPanen;
                    if (sisa <= 0 && !this.isEditMode) {
                        this.notify('error', 'Kapasitas Serapan Habis', 'Mohon maaf, sisa hasil panen untuk didistribusikan sudah habis.');
                        this.formSerapan.total_distribusi = '';
                        return;
                    }
                    if (inputSerapan > sisa) {
                        this.notify('warning', 'Melebihi Kapasitas', `Total serapan (${inputSerapan} Ton) melebihi sisa hasil panen yang tersedia (${sisa.toFixed(2)} Ton).`);
                    }
                },
                ...
                async submitTanam() {
                    ...
                    const requiredFields = [
                        ...
                        { key: 'luas_tanam', label: 'Luas Tanam (Ha)' },
                        { key: 'jenis_bibit', label: 'Jenis Bibit' },
                        { key: 'kebutuhan_bibit', label: 'Kebutuhan Bibit (Kg)' },
                        { key: 'est_awal_panen', label: 'Estimasi Awal Panen' },
                        { key: 'est_akhir_panen', label: 'Estimasi Akhir Panen' }
                    ];

                    for (const field of requiredFields) {
                        if (!this.formTanam[field.key]) {
                            this.notify('warning', 'Validasi Form', `Kolom ${field.label} wajib diisi! Mohon lengkapi data terlebih dahulu.`);
                            return;
                        }
                    }
                    ...
                    const sisaLahan = this.sisaLahan;
                    const inputLuas = parseFloat(this.formTanam.luas_tanam || 0);

                    if (sisaLahan <= 0 && !this.isEditMode) {
                        this.notify('error', 'Kapasitas Lahan Penuh', 'Mohon maaf, sisa kapasitas untuk lahan ini sudah habis/terpakai seluruhnya.');
                        return;
                    }

                    if (inputLuas > sisaLahan) {
                        this.notify('warning', 'Melebihi Kapasitas', `Luas tanam yang dimasukkan (${inputLuas} Ha) melebihi sisa kapasitas lahan yang tersedia (${sisaLahan.toFixed(2)} Ha).`);
                        return;
                    }
                    try {
                ...
                async submitPanen() {
                    ...
                    // Validasi field wajib Panen
                    const requiredPanenFields = [
                        { key: 'tgl_panen', label: 'Tanggal Panen' },
                        { key: 'status_panen', label: 'Jenis Panen' },
                        { key: 'luas_panen', label: 'Luas Panen (Ha)' },
                        { key: 'total_panen', label: 'Hasil Panen (Ton)' }
                    ];

                    for (const field of requiredPanenFields) {
                        if (this.formPanen[field.key] === '' || this.formPanen[field.key] === null || this.formPanen[field.key] === undefined) {
                            this.notify('warning', 'Validasi Form', `Kolom ${field.label} wajib diisi! Mohon lengkapi data terlebih dahulu.`);
                            return;
                        }
                    }
                    ...
                    const sisaPanenHa = parseFloat(this.activeLahan?.sisa_panen_ha ?? targetTanam?.luas_tanam ?? this.activeLahan?.luas_tanam ?? 0);

                    if (Number(this.formPanen.status_panen) !== 2 && sisaPanenHa > 0 && inputLuasPanen > sisaPanenHa) {
                        this.notify('warning', 'Validasi Gagal', `Luas panen (${inputLuasPanen} Ha) tidak boleh melebihi sisa luas tanam yang tersedia (${sisaPanenHa.toFixed(2)} Ha).`);
                        return;
                    }
                    try {
                        const url = this.isEditMode ?
                ...
                async submitSerapan() {
                    ...
                    // Validasi field wajib Serapan
                    const requiredSerapanFields = [
                        { key: 'tgl_distribusi', label: 'Tanggal Serapan' },
                        { key: 'total_distribusi', label: 'Total Serapan (Ton)' },
                        { key: 'distribusi_ke', label: 'Tujuan Serapan' }
                    ];

                    for (const field of requiredSerapanFields) {
                        if (this.formSerapan[field.key] === '' || this.formSerapan[field.key] === null || this.formSerapan[field.key] === undefined) {
                            this.notify('warning', 'Validasi Form', `Kolom ${field.label} wajib diisi! Mohon lengkapi data terlebih dahulu.`);
                            return;
                        }
                    }
                    ...
                    const sisaPanenSekarang = this.sisaPanen;
                    const inputSerapan = parseFloat(this.formSerapan.total_distribusi || 0);

                    if (sisaPanenSekarang <= 0 && !this.isEditMode) {
                        this.notify('error', 'Kapasitas Serapan Habis', 'Mohon maaf, sisa hasil panen untuk didistribusikan sudah habis.');
                        return;
                    }
                    if (inputSerapan > sisaPanenSekarang) {
                        this.notify('warning', 'Melebihi Kapasitas', `Total Serapan yang dimasukkan (${inputSerapan} Ton) melebihi sisa hasil panen yang tersedia (${sisaPanenSekarang.toFixed(2)} Ton).`);
                        return;
                    }
                ...
                async ajukanAkhiriSiklus(id) {
                    const ok = await this.askConfirm({
                        type: 'info',
                        title: 'Akhiri Siklus?',
                        message: 'Ajukan akhir siklus ke Admin?',
                        confirmText: 'Ya, Ajukan'
                    });
                    if (!ok) return;
                    try {
                        if (typeof NProgress !== 'undefined') NProgress.start();
                        const result = await this.fetchJson(`/${this.routeBase}/tanam/${id}/selesai`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                        ...
                        this.notify('success', 'Berhasil Diajukan', result.message || 'Pengajuan akhir siklus berhasil.');
                        setTimeout(() => window.location.reload(), 1500);
                    } catch (error) {
                        this.notify('error', 'Kesalahan Koneksi', error.message);
                    } finally {
                ...
                async terimaAkhiriSiklus(id) {
                    const ok = await this.askConfirm({
                        type: 'success',
                        title: 'Selesai Siklus?',
                        message: 'Siklus lahan ini akan diselesaikan dan dimasukkan ke Riwayat Lahan.',
                        confirmText: 'Ya, Selesai Siklus'
                    });
                    if (!ok) return;
                    try {
                        if (typeof NProgress !== 'undefined') NProgress.start();
                        const result = await this.fetchJson(`/${this.routeBase}/tanam/${id}/terima-siklus`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                        ...
                        if (result.success === false) {
                            this.notify('error', 'Gagal Menyetujui', result.message || 'Terjadi kesalahan server.');
                            return;
                        }
                        this.notify('success', 'Berhasil Disetujui', result.message || 'Siklus berhasil diarsipkan.');
                        setTimeout(() => window.location.reload(), 1500);
                    } catch (error) {
                        this.notify('error', 'Kesalahan Koneksi', error.message);
                    } finally {
                ...
                async sendTolakRequest(id, type, targetReject, alasan = '') {
                    let url = `/${this.routeBase}/${type}/${id}/tolak`;
                    if (type === 'Akhiri Siklus') {
                        url = `/${this.routeBase}/tanam/${id}/tolak-siklus`;
                    }
                    const result = await this.fetchJson(url, {
                        method: 'PUT',
                        headers: {
                        ...
                        body: JSON.stringify({
                            ...
                            alasan: alasan || '',
                            alasan_tolak_akhiri_siklus: alasan || '' // For backwards compatibility if needed
                        })
                    });
                    return result;
                },

                async submitValidasiDirect(id, type) {
                    try {
                        if(typeof NProgress !== 'undefined') NProgress.start();
                        const formData = new FormData();
                        formData.append('_method', 'PUT');

                        const response = await fetch(`/${this.routeBase}/${type}/${id}/validasi`, {
                            method: 'POST',
                            body: formData,
                            headers: {
                        ...
                        if (result.success) {
                            this.notify('success', 'Validasi Berhasil', result.message || 'Data berhasil divalidasi.');
                            setTimeout(() => window.location.reload(), 1500);
                        } else {
                            throw new Error(result.message || 'Gagal validasi data');
                        }
                    } catch (error) {
                        this.notify('error', 'Gagal Validasi', error.message);
                    } finally {
                        if(typeof NProgress !== 'undefined') NProgress.done();
                    }
                },
                ...
                async submitTolakAkhiriSiklus(id, lokasi) {
                    this.tolakModalData = {
                        isOpen: true,
                        id: id,
                        type: 'Akhiri Siklus',
            ...
            <div class="p-8 overflow-y-auto custom-scrollbar space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="number" step="0.01" x-model="formTanam.luas_tanam" @input="checkLuasTanamRealtime($event.target.value)" class="w-full text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-1">
    ...
    <div x-show="modalPanen"
        ...
        <div x-show="modalPanen"
            ...
            <div class="p-8 overflow-y-auto custom-scrollbar space-y-6">
                ...
                <div>
                    {{-- Info Estimasi Panen dari Proses Tanam --}}
                    <div class="mb-5 p-4 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-2xl flex items-center justify-between shadow-sm">
                        <div>
                            <p class="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1 flex items-center gap-1.5">
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                <span class="px-2.5 py-1 bg-white border border-emerald-200 text-slate-700 text-xs font-bold rounded-lg shadow-sm" x-text="activeTanamData?.est_awal_panen || '-'"></span>
                                <span class="text-[10px] font-black text-slate-400">s/d</span>
                                <span class="px-2.5 py-1 bg-white border border-emerald-200 text-slate-700 text-xs font-bold rounded-lg shadow-sm" x-text="activeTanamData?.est_akhir_panen || '-'"></span>
                            </div>
                        </div>
                        <div class="w-10 h-10 bg-emerald-100/50 rounded-xl flex items-center justify-center border border-emerald-200">
                            <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        </div>
                    </div>

                    {{-- Kapasitas Tanam Indicator --}}
                    <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl mb-5">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[10px] font-black text-amber-700 uppercase tracking-widest">📐 Sisa Luas Tanam</span>
                            <span class="text-[10px] font-black text-amber-800 bg-amber-200 px-2 py-0.5 rounded-lg" x-text="(activeLahan?.sisa_panen_ha ?? activeTanamData?.luas_tanam ?? 0) + ' Ha Tersedia'"></span>
                        </div>
                        <div class="text-[10px] font-bold text-amber-600 mb-1">
                            Masukkan luas panen ≤ sisa luas tanam.
                        </div>
                        <div class="mt-2 h-2 bg-amber-100 rounded-full overflow-hidden">
                            <div class="h-2 bg-gradient-to-r from-amber-400 to-orange-500 rounded-full transition-all duration-500"
                                :style="'width:' + Math.min(100, ((parseFloat(formPanen.luas_panen)||0) / Math.max(0.01, parseFloat(activeLahan?.sisa_panen_ha ?? activeTanamData?.luas_tanam||1))) * 100) + '%'"></div>
                        </div>
                        <div class="flex justify-between mt-1">
                            <span class="text-[9px] text-amber-500 font-bold">0 Ha</span>
                            <span class="text-[9px] font-black" :class="(parseFloat(formPanen.luas_panen)||0) > parseFloat(activeLahan?.sisa_panen_ha ?? activeTanamData?.luas_tanam||0) ? 'text-rose-600' : 'text-amber-700'" x-text="'Input: ' + (parseFloat(formPanen.luas_panen)||0).toFixed(2) + ' Ha'"></span>
                            <span class="text-[9px] text-amber-600 font-bold" x-text="'Sisa: ' + (activeLahan?.sisa_panen_ha ?? activeTanamData?.luas_tanam ?? 0) + ' Ha'"></span>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="number" step="0.01" x-model="formPanen.luas_panen" @input="checkLuasPanenRealtime($event.target.value)" class="w-full text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none transition-all">
                    </div>
    ...
    <div x-show="modalSerapan"
        ...
        <div x-show="modalSerapan"
            ...
            <div class="p-8 overflow-y-auto custom-scrollbar space-y-6">
                ...
                {{-- Info Sisa Panen --}}
                <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl mb-5">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-[10px] font-black text-blue-700 uppercase tracking-widest">🌾 Sisa Hasil Panen</span>
                        <span class="text-[10px] font-black text-blue-800 bg-blue-200 px-2 py-0.5 rounded-lg" x-text="parseFloat(activeLahan?.sisa_panen || 0).toFixed(2) + ' Ton Tersedia'"></span>
                    </div>
                    <div class="text-[10px] font-bold text-blue-600 mb-1">
                        Masukkan serapan ≤ sisa hasil panen.
                    </div>
                    <div class="mt-2 h-2 bg-blue-100 rounded-full overflow-hidden">
                        <div class="h-2 bg-gradient-to-r from-blue-400 to-indigo-500 rounded-full transition-all duration-500"
                            :style="'width:' + Math.min(100, ((parseFloat(formSerapan.total_distribusi)||0) / Math.max(0.01, parseFloat(activeLahan?.sisa_panen||1))) * 100) + '%'"></div>
                    </div>
                    <div class="flex justify-between mt-1">
                        <span class="text-[9px] text-blue-500 font-bold">0 Ton</span>
                        <span class="text-[9px] font-black" :class="(parseFloat(formSerapan.total_distribusi)||0) > parseFloat(activeLahan?.sisa_panen||0) ? 'text-rose-600' : 'text-blue-700'" x-text="'Input: ' + (parseFloat(formSerapan.total_distribusi)||0).toFixed(2) + ' Ton'"></span>
                        <span class="text-[9px] text-blue-600 font-bold" x-text="'Sisa: ' + parseFloat(activeLahan?.sisa_panen || 0).toFixed(2) + ' Ton'"></span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mt-4">
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="number" step="0.01" placeholder="0.00" x-model="formSerapan.total_distribusi" @input="checkSerapanRealtime($event.target.value)" class="w-full text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all">
                    </div>
                </div>
                <div>
    ...
    <div x-show="modalValidasi"
        ...
        <div x-show="modalValidasi"
             ...
             x-transition:leave-start="opacity-100 scale-100 translate-y-0"
             x-transition:leave-end="opacity-0 scale-95 translate-y-4"
             class="relative z-10 bg-white rounded-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]"
             style="box-shadow: 0 32px 80px -16px rgba(0,0,0,0.32), 0 0 0 1px rgba(0,0,0,0.05);">
                        ...
                        <template x-for="t in validasiData.tanam" :key="t.id_tanam">
                            <div class="bg-emerald-50 border border-emerald-100 rounded-xl p-3 flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" @click="submitValidasiDirect(t.id_tanam, 'tanam')" class="px-2 py-1 bg-emerald-500 text-white rounded shadow-sm text-[10px] font-bold hover:bg-emerald-600">Validasi</button>
                        ...
                        <template x-for="p in validasiData.panen" :key="p.id_panen">
                            <div class="bg-amber-50 border border-amber-100 rounded-xl p-3 flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" @click="submitValidasiDirect(p.id_panen, 'panen')" class="px-2 py-1 bg-amber-500 text-white rounded shadow-sm text-[10px] font-bold hover:bg-amber-600">Validasi</button>
                        ...
                        <template x-for="s in validasiData.serapan" :key="s.id_distribusi">
                            <div class="bg-blue-50 border border-blue-100 rounded-xl p-3 flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" @click="submitValidasiDirect(s.id_distribusi, 'serapan')" class="px-2 py-1 bg-blue-500 text-white rounded shadow-sm text-[10px] font-bold hover:bg-blue-600">Validasi</button>
    ...
    <div x-show="detailModalData.isOpen"
        ...
        <div x-show="detailModalData.isOpen"
             ...
             x-transition:leave-start="opacity-100 scale-100 translate-y-0"
             x-transition:leave-end="opacity-0 scale-95 translate-y-4"
             class="relative z-10 bg-white rounded-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]"
             style="box-shadow: 0 32px 80px -16px rgba(0,0,0,0.32), 0 0 0 1px rgba(0,0,0,0.05);">
    ...
    <!-- MODAL TOLAK VALIDASI -->
        <div x-show="tolakModalData.isOpen"
            ...
            <div x-show="tolakModalData.isOpen"
                 ...
                 x-transition:leave-start="opacity-100"
                 x-transition:leave-end="opacity-0"
                 @click="tolakModalData.isOpen = false"
                 class="absolute inset-0"
                 style="background: rgba(15,23,42,0.62); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px);"></div>
            <!-- Dialog -->
            <div x-show="tolakModalData.isOpen"
                 ...
                 x-transition:leave-start="opacity-100 scale-100 translate-y-0"
                 x-transition:leave-end="opacity-0 scale-95 translate-y-4"
                 class="relative z-10 bg-white rounded-2xl w-full max-w-lg overflow-hidden flex flex-col"
                 style="box-shadow: 0 32px 80px -16px rgba(0,0,0,0.32), 0 0 0 1px rgba(0,0,0,0.05);">
    ...
    <div @keydown.escape.window="modalJadwalPanen = false"
        ...
        <div x-show="modalJadwalPanen"
             ...
             class="relative bg-white rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col m-4"
             style="box-shadow: 0 32px 80px -16px rgba(0,0,0,0.32), 0 0 0 1px rgba(0,0,0,0.05);">

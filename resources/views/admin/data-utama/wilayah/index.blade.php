...
@section('content')
    <div class="space-y-8 pb-20 antialiased text-slate-900"
        ...
        <div
            class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-4 mb-10 transition-all duration-700 animate-in fade-in slide-in-from-top-4">
            <div>
                ...
                <h2 class="text-3xl lg:text-4xl font-semibold text-slate-800 tracking-tight">
                    Data <span class="text-blue-600 font-normal">Wilayah</span>
                </h2>
            </div>
        ...
        <div class="space-y-4">
            @forelse($kabupatenList as $kab)
                ...
                <div x-data="{ open: false }"
                    class="bg-white/80 backdrop-blur-xl rounded-[2.2rem] border border-white shadow-lg overflow-hidden">
                    {{-- Header Kabupaten --}}
                    <div @click="open = !open"
                        class="px-8 py-5 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-all">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-lg font-black text-slate-800 uppercase italic tracking-tight leading-none">
                                    {{ $kab->nama_wilayah }}</h3>
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">KODE:
                                    {{ $kab->kode_wilayah }}</p>
                            </div>
                        </div>
                        <svg class="w-5 h-5 text-slate-400 transition-transform duration-300" :class="open ? 'rotate-180' : ''"
                            ...
                            @foreach($kecamatanInKab as $kec)
                                <div x-data="{ subOpen: false }"
                                    ...
                                    <div @click="subOpen = !subOpen"
                                        class="px-6 py-4 flex justify-between items-center cursor-pointer hover:bg-blue-50/30 transition-all">
                                        <div class="flex items-center gap-3">
                                            <div class="w-2 h-2 bg-blue-500 rounded-full"></div>
                                            <h4 class="text-[12px] font-black text-slate-700 uppercase tracking-wide">
                                                {{ $kec->nama_wilayah }}</h4>
                                        </div>
                                        <span
                                            class="text-[9px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-lg uppercase tracking-widest">Kecamatan</span>
                                    </div>

                                    {{-- Content Desa --}}
                                    <div x-show="subOpen" x-collapse class="px-6 pb-4">
                                        <div
                                            class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 pt-2 border-t border-slate-100">
                                            @php
                                            ...
                                            @forelse($desaList as $desa)
                                                <div
                                                    class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 group hover:border-blue-200 transition-all">
                                                    <svg class="w-4 h-4 text-slate-300 group-hover:text-blue-500" fill="none"
                                                    ...
                                                    <span
                                                        class="text-[11px] font-bold text-slate-600 uppercase group-hover:text-slate-900">{{ $desa->nama_wilayah }}</span>
                                                </div>
                                            @empty
                                                <p class="col-span-full text-[10px] text-slate-400 italic py-2">Tidak ada data desa di
                                                    kecamatan ini.</p>
                                            @endforelse
                                        </div>
                                    </div>
            ...
            @empty
                <div class="bg-white rounded-[2.5rem] p-20 text-center border border-slate-200">
                    <p class="text-slate-400 uppercase font-black tracking-widest text-xs">Belum ada data wilayah yang tersedia.
                    </p>
                </div>
            @endforelse
        </div>

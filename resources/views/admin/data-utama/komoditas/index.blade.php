...
@section('content')
    <div class="space-y-8 pb-20 antialiased text-slate-900"
        ...
        <div
            class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 px-2 transition-all mb-10 duration-700 animate-in fade-in slide-in-from-top-4">
            <div>
                <nav class="flex items-center text-xs gap-2 font-medium text-slate-400 mb-1">
                ...
                <h2 class="text-3xl lg:text-4xl font-semibold text-slate-800 tracking-tight">
                    Data <span class="text-indigo-600 font-normal">Komoditas</span>
                </h2>
            </div>
        ...
        @forelse($groupedKomoditi as $jenis => $items)
            <div x-data="{ open: true }"
                class="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/40 overflow-hidden mb-8">
                {{-- Category Header --}}
                <div @click="open = !open"
                    class="bg-slate-900/[0.02] px-10 py-5 border-b border-slate-100 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-all">
                        ...
                        <table class="w-full text-left border-separate border-spacing-0">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-200">
                                    ...
                                    <th
                                        class="px-10 py-6 text-center text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">
                                        Aksi</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="group hover:bg-slate-50 transition-all duration-200">
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="flex items-center gap-4">
                                                <div
                                                    class="w-11 h-11 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 font-black text-[10px] border border-slate-200 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-500">
                                                    {{ substr($item->nama_komoditi, 0, 2) }}
                                                </div>
                                                <h4
                                                    class="text-[13px] font-black text-slate-800 uppercase italic tracking-tight group-hover:text-indigo-600">
                                                    {{ $item->nama_komoditi }}
                                                </h4>
                                            </div>
                                        </td>
                                        <td class="px-6 py-5">
                                            <span
                                                class="text-[11px] font-black text-slate-400 bg-slate-100 px-2.5 py-1 rounded-lg">#{{ $item->id_komoditi }}</span>
                                        </td>
                                        <td class="px-6 py-5">
                                        ...
                                        <td class="px-10 py-5 text-right">
                                            <div
                                                ...
                                                <button
                                                    class="p-2 bg-white text-indigo-500 rounded-lg border border-slate-200 hover:bg-indigo-600 hover:text-white shadow-sm transition-all">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"

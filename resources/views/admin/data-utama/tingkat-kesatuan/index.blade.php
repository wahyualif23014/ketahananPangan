...
<div class="space-y-6 pb-20 font-sans" style="font-family: 'Ramabhadra', sans-serif;">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div class="group relative overflow-hidden bg-white/60 backdrop-blur-xl p-5 rounded-[1.8rem] border border-white shadow-xl shadow-slate-200/40 transition-all hover:-translate-y-1">
            <div class="flex items-center gap-4 relative z-10">
                <div class="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center border border-emerald-100 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-500">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    </svg>
                </div>
                <div>
                    <p class="text-slate-400 text-[8px] font-black uppercase tracking-[0.3em] mb-0.5">Cakupan Wilayah</p>
                    <h3 class="text-lg font-black text-slate-800 uppercase italic leading-none">
                        Terdapat <span class="text-emerald-600 text-2xl tracking-tighter">{{ $kecamatanList->count() }}</span> Kecamatan
                    </h3>
                </div>
            </div>
        ...
        <div class="group relative overflow-hidden bg-white/60 backdrop-blur-xl p-5 rounded-[1.8rem] border border-white shadow-xl shadow-slate-200/40 transition-all hover:-translate-y-1">
            <div class="flex items-center gap-4 relative z-10">
                <div class="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <h3 class="text-lg font-black text-slate-800 uppercase italic leading-none">
                        Terdapat <span class="text-blue-600 text-2xl tracking-tighter">{{ $totalKabupaten }}</span> Kabupaten
                    </h3>
                </div>
            </div>
    ...
    <div class="bg-white/80 backdrop-blur-xl rounded-[2.2rem] border border-white shadow-2xl shadow-slate-200/40 overflow-hidden">
        <div class="grid grid-cols-12 bg-slate-900/[0.02] border-b border-slate-100">
            <div class="col-span-8 px-10 py-5 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Struktur Kecamatan & Kabupaten</div>
            <div class="col-span-4 px-10 py-5 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] text-right">Kode Wilayah</div>
        </div>
        ...
        <div x-data="{ expanded: false }" class="group/parent border-b border-slate-50 last:border-0">
            <div @click="expanded = !expanded" :class="expanded ? 'bg-emerald-50/20' : 'bg-white hover:bg-slate-50'"
                ...
                <div class="col-span-8 px-10 py-7">
                    <div class="flex items-center gap-5">
                        ...
                        <div class="space-y-1">
                            <h3 class="text-lg font-black text-slate-800 uppercase italic tracking-tight group-hover/parent:text-emerald-600 transition-colors leading-none">
                                {{ $kec->nama_wilayah }}
                            </h3>
                            <div class="flex flex-wrap items-center gap-x-4 text-[9px] font-bold uppercase tracking-widest text-slate-400">
                                <span class="flex items-center gap-1.5 bg-white px-2 py-0.5 rounded-lg border border-slate-100">
                                ...
                                <span class="flex items-center gap-1.5 text-emerald-600">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 004.815 4.815l.773-1.548a1 1 0 011.06-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z"></path>
                                    </svg>
                                    {{ $pj->no_telp_anggota ?? '-' }}
                                </span>
                            </div>
                ...
                <div class="col-span-4 px-10 text-right">
                    <span class="inline-flex items-center px-4 py-1.5 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase tracking-[0.15em] shadow-md shadow-emerald-200">
            ...
            <div x-show="expanded" x-collapse>
                <div class="relative bg-slate-50/40">
                    <div class="absolute left-[3.1rem] top-0 bottom-10 w-[2px] bg-gradient-to-b from-emerald-200 to-transparent opacity-40"></div>
                    ...
                    <div class="border-t border-white/60">
                        <div class="grid grid-cols-12 items-center hover:bg-white transition-all duration-300 group/child">
                            <div class="col-span-8 px-24 py-5 relative">
                                <div class="absolute left-[3.1rem] top-1/2 -translate-y-1/2 -translate-x-1/2 w-3.5 h-3.5 bg-white border-[3px] border-emerald-400 rounded-full shadow-sm group-hover/child:bg-emerald-500 group-hover/child:border-white transition-all"></div>

                                <div class="space-y-1">
                                    <h4 class="text-[13px] font-black text-slate-700 uppercase italic tracking-wide group-hover/child:text-emerald-600 transition-colors leading-none">
                                        {{ $kab->nama_wilayah }}
                                    </h4>
                                    <div class="flex items-center gap-3 text-[9px] font-bold uppercase tracking-widest text-slate-400">
                                        <span>DESA TERDAFTAR: <span class="text-slate-600">{{ $desaList->count() }}</span></span>
                                        <span class="text-slate-200">/</span>
                                        <span class="italic text-slate-500 font-medium">PJ: {{ $pjKab->nama_anggota ?? 'N/A' }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-span-4 px-10 text-right">
                                <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.1em] bg-white border border-slate-100 px-3 py-1 rounded-lg shadow-sm group-hover/child:text-emerald-600 transition-all">
                                    {{ $kab->kode_wilayah }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="px-32 pb-4 space-y-2">
                            ...
                            <div class="flex items-center justify-between bg-white/40 p-2 rounded-lg border border-slate-100">
                                <span class="text-[10px] font-bold text-slate-500 uppercase">{{ $desa->nama_wilayah }}</span>

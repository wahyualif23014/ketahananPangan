...
@section('content')
{{-- 
    PENTING: Gunakan tanda kutip tunggal (') untuk x-data 
    supaya tidak tabrakan dengan tanda kutip ganda (") dari @json
--}}
<div x-data='{ 
    showModal: false, 
    selectAll: false, 
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 px-2">
        <div>
            <nav class="flex items-center gap-2 text-[10px] font-black tracking-[0.2em] uppercase text-slate-400 mb-1">
                ...
                <span class="text-blue-600 italic">Jabatan Anggota</span>
            </nav>
            <h2 class="text-3xl font-black text-slate-800 tracking-tighter uppercase italic leading-none">
                Master <span class="text-blue-600">Jabatan</span>
            </h2>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                <button @click="window.location.reload()" title="Refresh" class="p-3 bg-white text-emerald-600 rounded-2xl shadow-sm border border-slate-200 hover:bg-emerald-50 transition-all active:scale-90">
                ...
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        <th class="w-12 pl-8 py-6 text-center">
                            <input type="checkbox" @click="toggleAll()" :checked="selectAll" class="w-4 h-4 rounded border-slate-300 text-blue-600">
                        </th>
                    ...
                    <template x-for="item in filteredItems" :key="item.id_jabatan">
                        <tr class="group hover:bg-slate-50 transition-all duration-200">
                            <td class="w-12 pl-8 py-5 text-center">
                                <input type="checkbox" :value="item.id_jabatan" x-model="selected" class="w-4 h-4 rounded border-slate-300 text-blue-600">
                            </td>
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-4">
                                    <div class="w-11 h-11 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 font-black text-[10px] border border-slate-200 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500" x-text="item.nama_jabatan.substring(0,2)"></div>
                                    <h4 class="text-[13px] font-black text-slate-800 uppercase italic tracking-tight group-hover:text-blue-600" x-text="item.nama_jabatan"></h4>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <span class="text-[11px] font-black text-slate-400 bg-slate-100 px-2.5 py-1 rounded-lg" x-text="'#' + item.id_jabatan"></span>
                            </td>
                            <td class="px-6 py-5">
                                <span class="text-[10px] font-bold text-slate-400 italic" x-text="item.created_at_formatted || '-'"></span>
                            </td>
                            <td class="px-8 py-5">
                                <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                    <button class="p-2 bg-white text-blue-500 rounded-lg border border-slate-200 hover:bg-blue-600 hover:text-white shadow-sm transition-all">

...
@section('content')
@php
// Logika Grouping Data Personel berdasarkan kesatuan
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/40 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-separate border-spacing-0">
                ...
                {{-- PERUBAHAN UTAMA: Membungkus dengan <tbody> --}}
                @forelse($groupedPersonels as $unit => $members)
                <tbody x-data="{ expanded: false }" class="divide-y divide-slate-100">
                    {{-- Dropdown Header Grup --}}
                    <tr class="bg-slate-100/80 hover:bg-slate-200/80 cursor-pointer transition-colors border-y border-slate-200" @click="expanded = !expanded">
                    ...
                    <tr x-show="expanded" x-transition class="group hover:bg-slate-200/70 transition-all duration-200">
                        ...
                        <td class="px-8 py-5 text-center border-none">
                            <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                                <button class="p-2 bg-white text-blue-500 rounded-lg border border-slate-200 hover:bg-blue-600 hover:text-white shadow-sm transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5M16.5 3.5a2.121 2.121 0 113 3L12 15l-4 1 1-4 9.5-9.5z" stroke-width="2"></path>
                                    </svg></button>
                                <button class="p-2 bg-white text-rose-500 rounded-lg border border-slate-200 hover:bg-rose-600 hover:text-white shadow-sm transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody>
                    <tr>
                        <td colspan="6" class="px-8 py-10 text-center text-slate-400 text-sm font-bold uppercase tracking-wider">
                            Belum ada data personel yang terdaftar di sistem.
                        </td>
                    </tr>
                </tbody>
                @endforelse

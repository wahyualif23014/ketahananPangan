...
@php
    ...
    /**
     * OPTIMASI 1: Pre-calculating & Grouping
     * Kita melakukan grouping dan kalkulasi total di sini satu kali saja.
     * Ini mencegah Blade melakukan loop sum() berulang-ulang di dalam tabel.
     */
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-4 transition-all mb-10 duration-700 animate-in fade-in slide-in-from-top-4">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                <button onclick="window.location.reload()" title="Refresh" class="p-2.5 text-slate-500 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 hover:text-emerald-600 transition-all active:scale-95 shadow-sm">
                ...
                <button type="submit" form="form-filter" formaction="{{ route($routePrefix . '.rekapitulasi.export') }}" class="inline-flex items-center px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-xl transition-all shadow-sm">
    ...
    <form action="{{ url()->current() }}" method="GET" class="mx-4 bg-white border border-slate-200 rounded-xl shadow-sm" x-data="{ open: true }">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg transition-all shadow-sm">Terapkan Filter</button>
        ...
        <div x-show="open" x-collapse x-cloak class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="space-y-4" x-data="{
                    ...
                    polresLabel: '{{ request('polres') ? optional($polresList->firstWhere('id_tingkat', request('polres')))->nama_tingkat : '' }}',
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative">
                            ...
                            <div class="relative">
                                ...
                                <div x-show="polresLabel && !polresOpen" @click="polresOpen = true" class="w-full h-10 pl-9 bg-slate-50 border border-slate-200 rounded-lg text-sm flex items-center cursor-pointer hover:bg-white transition-all">
                        ...
                        <div class="relative">
                            ...
                            <div class="relative">
                                <input type="text" x-show="polresValue && (!polsekLabel || polsekOpen)" x-model="polsekSearch" @focus="polsekOpen = true" @click.away="polsekOpen = false" :disabled="polsekLoading || !polresValue" placeholder="Cari polsek..."
                                    class="w-full h-10 pl-9 pr-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white transition-all outline-none focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/5">
                                <div x-show="polsekLabel && !polsekOpen" @click="polsekOpen = true" class="w-full h-10 pl-9 bg-slate-50 border border-slate-200 rounded-lg text-sm flex items-center cursor-pointer hover:bg-white transition-all">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                        @php
                            $filterSpecs = [
                                ['label' => 'Jenis Lahan', 'name' => 'jenis_lahan', 'items' => $jenisLahanList->map(fn($i) => ['value' => $i->id_jenis_lahan, 'label' => $i->nama_jenis_lahan]), 'request_key' => 'jenis_lahan'],
                                ['label' => 'Komoditi', 'name' => 'komoditi', 'items' => $komoditiList->map(fn($i) => ['value' => $i->id_komoditi, 'label' => $i->nama_komoditi]), 'request_key' => 'komoditi']
                            ];
                        @endphp
                        @foreach($filterSpecs as $spec)
                        <div x-data="{
                            isOpen: false, search: '', selectedValue: '{{ request($spec['request_key'], '') }}',
                            selectedLabel: '{{ request($spec['request_key']) ? optional($spec['items']->firstWhere('value', request($spec['request_key'])))['label'] : '' }}',
                            items: @js($spec['items']),
                            ...
                            <div class="relative">
                                <input type="text" x-show="!selectedLabel || isOpen" x-model="search" @focus="isOpen = true" @click.away="isOpen = false" placeholder="Cari {{ strtolower($spec['label']) }}..."
                                    class="w-full h-10 pl-9 pr-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white transition-all outline-none focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/5">
                            ...
                            <div x-show="isOpen && filtered.length > 0" class="absolute z-50 left-0 right-0 mt-1.5 bg-white border border-slate-200 rounded-lg shadow-xl max-h-48 overflow-y-auto" x-cloak>
                                <template x-for="item in filtered" :key="item.value">
                                    <div @mousedown.prevent="selectedValue = item.value; selectedLabel = item.label; isOpen = false; $nextTick(() => $el.closest('form').submit())" class="px-3.5 py-2.5 text-sm cursor-pointer hover:bg-emerald-50" x-text="item.label"></div>
                                </template>
                            </div>
                        </div>
                        @endforeach
    ...
    <div class="mx-4 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-bold rounded-full uppercase tracking-wide">{{ $dataRekap->total() }} baris data</span>
            </div>
            ...
            <table class="w-full text-left border-separate border-spacing-0">
                <thead>
                    <tr class="bg-slate-50/80 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                        <th rowspan="2" class="px-8 py-4 border-b-2 border-slate-200 text-left font-bold">Wilayah / Satker</th>
                        <th rowspan="2" class="px-6 py-4 text-right border-b-2 border-slate-200 whitespace-nowrap">Potensi Lahan</th>
                        <th rowspan="2" class="px-6 py-4 text-right border-b-2 border-slate-200 whitespace-nowrap">Aktual Tanam</th>
                ...
                <div x-show="!openPolsek['{{ $polsekName }}']" class="p-3 space-y-2">
                    ...
                    <div class="rounded-xl border border-slate-200 p-3 bg-white space-y-2 shadow-sm">
                        ...
                        <div class="grid grid-cols-3 gap-2 text-center text-[10px]">
                            ...
                            <div class="bg-slate-50 p-1 rounded">Serap<br><b class="text-emerald-600">{{ number_format($row->persentase_serapan, 1) }}%</b></div>
                        </div>
                    </div>
                    @endforeach
                </div>

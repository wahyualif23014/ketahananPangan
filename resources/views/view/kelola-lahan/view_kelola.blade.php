...
@section('header', 'Data Produksi Lahan (Viewer)')
...
<div class="space-y-8 pb-24 kelola-container max-w-[1600px] mx-auto" x-data="kelolaLahan()">
                    ...
                    <tr x-show="isResorOpen('{{ $resorId }}')"
                        ...
                        <td class="px-4 py-6 border-x border-slate-50 align-top">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="flex flex-wrap items-center gap-1 mt-1">
                                    <button @click='editTanam("{{ $row->id_tanam }}", {!! json_encode($row) !!})' class="px-2 py-1 bg-white border border-emerald-200 text-emerald-600 rounded text-[9px] font-black uppercase hover:bg-emerald-500 hover:text-white transition-colors shadow-sm">Edit</button>
                                </div>
                            </div>
                            @else
                        ...
                        <td class="px-4 py-6 border-r border-slate-50 align-top">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="flex flex-wrap items-center gap-1 mt-1">
                                    <button @click='editPanen("{{ $row->id_panen }}", {!! json_encode($row) !!})' class="px-2 py-1 bg-white border border-amber-200 text-amber-600 rounded text-[9px] font-black uppercase hover:bg-amber-500 hover:text-white transition-colors shadow-sm">Edit</button>
                                </div>
                            </div>
                            @else
                            <span class="text-[10px] font-bold text-slate-400 italic">Belum Input</span>
                        ...
                        <td class="px-4 py-6 border-r border-slate-50 align-top">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="flex flex-wrap items-center gap-1 mt-1">
                                    <button @click='editSerapan("{{ $row->id_distribusi }}", {!! json_encode($row) !!})' class="px-2 py-1 bg-white border border-blue-200 text-blue-600 rounded text-[9px] font-black uppercase hover:bg-blue-500 hover:text-white transition-colors shadow-sm">Edit</button>
                                </div>
                        ...
                        <td class="px-6 py-6">
                            <div class="flex flex-col gap-3 min-w-[160px]">
                                ...
                                <template x-if="lahanStages['{{ $row->id_lahan }}'] === 0">
                                    <button @click='openStageModal("{{ $row->id_lahan }}", {!! json_encode($row) !!})'
    ...
    <script>
        function kelolaLahan() {
            return {
                periodMode: <?= json_encode(($filters['start'] || $filters['end']) ? 'tanggal' : 'semua') ?>,
                searchQuery: <?= json_encode($filters['search'] ?? '') ?>,
                selectedResor: <?= json_encode($filters['resor'] ?? '') ?>,
                selectedSektor: <?= json_encode($filters['sektor'] ?? '') ?>,
                selectedJenis: <?= json_encode($filters['jenis'] ?? '') ?>,
                selectedKomoditi: <?= json_encode($filters['komoditi'] ?? '') ?>,
                kategoriProduksi: <?= json_encode($filters['kategori'] ?? 'semua') ?>,
                polseks: <?= json_encode($polsekList) ?>,
                openResors: [],
                ...
                modalPanen: false,
                modalSerapan: false,
                lahanStages: <?= json_encode($lahanStagesMap ?? new stdClass()) ?>,
                ...
                async submitSerapan() {
                    try {
                        const url = this.isEditMode ? `/view/kelola-lahan/serapan/${this.activeProcessId}` : "{{ route('view.kelola-lahan.serapan.store') }}";
                        const method = this.isEditMode ? 'PUT' : 'POST';
                        const response = await fetch(url, {
                            method: method,
                            headers: {

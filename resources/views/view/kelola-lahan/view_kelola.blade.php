...
<div class="space-y-8 pb-24 kelola-container max-w-[1600px] mx-auto" x-data="kelolaLahan()">
                    ...
                    <tr x-show="isResorOpen('{{ $resorId }}')"
                        ...
                        <td class="px-4 py-6 border-r border-slate-50 align-top">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <span class="text-[9px] font-black text-slate-400 tracking-tight mt-1">⏳ Menunggu Validasi</span>
                                @else
                        ...
                        <td class="px-4 py-6 text-right">
                            <div class="flex flex-col items-end gap-1.5">
                                <a href="{{ route('view.kelola-lahan.potensi.index') }}?search={{ $row->id_lahan }}&action=view"
                                    title="Detail Lahan"
                                    class="p-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white rounded-lg transition-all shadow-sm inline-flex items-center justify-center">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @php
    // Precompute resor IDs for Alpine hydration — ensures accordion is open by default
    $resorIds = [];
    foreach ($data->items() as $r) {
    $resorIds[] = str_replace('.', '_', $r->id_tingkat);
    }
    $jsData = [
    'periodMode' => ($filters['start'] || $filters['end']) ? 'tanggal' : 'semua',
    'searchQuery' => $filters['search'] ?? '',
    'selectedResor' => $filters['resor'] ?? '',
    'selectedSektor' => $filters['sektor'] ?? '',
    'selectedJenis' => $filters['jenis'] ?? '',
    'selectedKomoditi' => $filters['komoditi'] ?? '',
    'kategoriProduksi' => $filters['kategori'] ?? 'semua',
    'polseks' => $polsekList,
    'lahanStages' => $lahanStagesMap ?? (object)[],
    'initialOpenResors'=> $resorIds,
    ];
    @endphp
    <script id="kelola-data" type="application/json">
        {
            !!json_encode($jsData) !!
        }
    </script>
    ...
    <script>
        function kelolaLahan() {
            const _d = JSON.parse(document.getElementById('kelola-data').textContent);
            return {
                periodMode: _d.periodMode,
                searchQuery: _d.searchQuery,
                selectedResor: _d.selectedResor,
                selectedSektor: _d.selectedSektor,
                selectedJenis: _d.selectedJenis,
                selectedKomoditi: _d.selectedKomoditi,
                kategoriProduksi: _d.kategoriProduksi,
                polseks: _d.polseks,
                lahanStages: _d.lahanStages,
                openResors: [],

                init() {
                    this.openResors = [..._d.initialOpenResors];
                },
                ...
                toggleResor(id) {
                    if (this.openResors.includes(id)) {
                        ...
                        this.openResors = [...this.openResors, id];
                    }
                },

...
<aside :class="{
    ...
    <div x-show="!desktopCollapsed" x-cloak x-transition
        ...
        <div class="relative inline-block mb-4">
            <div
                ...
                <span
                    class="-rotate-3">{{ collect(explode(' ', Auth::user()->name))->map(fn($n) => $n[0])->take(2)->implode('') }}</span>
            </div>
            <div class="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-slate-900 rounded-full">
            </div>
        </div>
    ...
    <nav class="flex-1 px-3 space-y-1 overflow-y-auto custom-scrollbar py-4">
        ...
        {{-- Dropdown: Data Utama --}}
        <div x-data="{ open: {{ request()->is('admin/data-utama*') ? 'true' : 'false' }} }">
        ...
        {{-- Section: Master Data --}}
        @hasrole('admin')
        <div x-show="!desktopCollapsed" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"

...
<div class="space-y-8 pb-24 potensi-container max-w-7xl mx-auto" x-data="potensiLahanManager()"
    ...
    @open-detail-modal.window="activeDetailData = $event.detail; isDetailOpen = true; activeTab = 'personel'"
    @close-all-modals.window="isDetailOpen = false; isOpen = false">
        ...
        <div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2 mt-1 sm:mt-0">
                    ...
                    <a href="{{ route('operator.kelola-lahan.potensi.index') }}" class="text-[10px] font-black text-rose-500 hover:text-rose-700 bg-white border border-slate-200 px-2.5 py-1.5 rounded-xl transition-all shadow-sm">
                        BATAL
                    </a>
                </div>
                @endif
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-200/60 shadow-2xl shadow-slate-200/50 overflow-hidden relative z-20 mx-2 mt-12">
        ...
        <div class="px-8 py-6 bg-gradient-to-r from-slate-900 to-slate-800 flex justify-between items-center relative overflow-hidden">
            ...
            <div class="flex items-center gap-3 relative z-10">
                ...
                <a href="{{ route('operator.kelola-lahan.potensi.index') }}" class="text-[10px] font-black text-rose-300 bg-rose-500/20 px-2 py-1 rounded-lg border border-rose-400/30 hover:bg-rose-500/40 transition-colors">
                    ✕ Hapus Filter
                </a>
                @endif
            </div>
        ...
        <div class="px-8 py-5 bg-white border-b border-slate-100 relative z-10">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-6">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-end mt-4 lg:mt-0">
                                <a href="{{ route('operator.kelola-lahan.potensi.index') }}" class="flex items-center gap-2 px-5 py-2.5 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-100 transition-all text-[10px] font-black uppercase tracking-widest border border-rose-100 shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                    </svg>
                                    Reset Filter
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td class="px-4 py-4 w-36">
                                        ...
                                        @php $alasanTolak = $item['ket_polisi'] ?? ''; @endphp
                                        @if($alasanTolak)
                                        <div class="text-[8px] text-rose-400 mt-1 italic line-clamp-2" title="{{ $alasanTolak }}">{{ $alasanTolak }}</div>
                                        @endif
                                    ...
                                    <td class="px-4 py-4 w-72">
                                        <div class="flex items-center justify-end gap-1.5 flex-wrap">
                                            ...
                                            @php
                                            $userDots = substr_count((string)auth()->user()->id_tugas, '.');
                                            @endphp
                                            @if($item['status_lahan'] == '2')
                                                {{-- DITOLAK: Hanya tampil Perbaiki Data untuk operator yg buat (polsek) --}}
                                                @if($userDots >= 2)
                                                <button onclick='openEditModal(@json($item))'
                                                    class="inline-flex items-center gap-1 text-[10px] font-black text-orange-600 bg-orange-50 border border-orange-200 px-2.5 py-1.5 rounded-lg hover:bg-orange-500 hover:text-white transition-all animate-pulse">
                                                    ✏️ Perbaiki Data
                                                </button>
                                                @else
                                                {{-- Polres: hanya lihat status menunggu --}}
                                            ...
                                            @elseif($userDots < 2)
                                                ...
                                                <div class="flex flex-col gap-1 w-full mt-1">
                                                    <form action="{{ route('operator.kelola-lahan.potensi.validasi', $item['id_lahan']) }}" method="POST" class="w-full m-0">
                                                    @csrf @method('PUT')
                                                ...
                                                @else
                                                <form action="{{ route('operator.kelola-lahan.potensi.unvalidasi', $item['id_lahan']) }}" method="POST" class="inline m-0" onsubmit="return handleFormConfirm(event, 'Batalkan Validasi?', 'Yakin ingin membatalkan validasi data lahan ini?', 'warning')">
                                                    @csrf @method('PUT')
                                                ...
                                                @endif
                                            @endif
                                            @if($item['status_lahan'] == '0' && $userDots >= 2)
                                              <button onclick='openEditModal(@json($item))'
                                              ...
                                              <form action="{{ route('operator.kelola-lahan.potensi.destroy', $item['id_lahan']) }}" method="POST" class="inline m-0" onsubmit="return handleFormConfirm(event, 'Hapus Data Lahan?', 'Data ini akan dihapus dari sistem. Tindakan ini tidak dapat dibatalkan.', 'danger')">
                                                  @csrf @method('DELETE')
            ...
            <script>
                ...
                function switchTab(tabId) {
                    ['tab-personel', 'tab-teknis', 'tab-lokasi', 'tab-validasi'].forEach(function(t) {
                        ...
                        btn.classList.remove('bg-white', 'text-emerald-600', 'shadow-sm', 'border', 'border-emerald-100/50');
                    ...
                    activeBtn.classList.add('bg-white', 'text-emerald-600', 'shadow-sm', 'border', 'border-emerald-100/50');
                    activeBtn.classList.remove('text-slate-500', 'hover:text-slate-700', 'hover:bg-white/60');
                }
                ...
                document.addEventListener('alpine:init', () => {
                    Alpine.data('tolakModal', () => ({
                        ...
                        async submitTolak() {
                            this.loading = true;
                            try {
                                const response = await fetch(`/operator/kelola-lahan/potensi/tolak/${this.id}`, {
                                    method: 'PUT',
                                    headers: {
            ...
            <!-- 1. VIEW MODAL (ALPINE JS - PREMIUM VERSION) -->
            <template x-teleport="body">
                <div x-show="isDetailOpen" 
                     class="fixed inset-0 z-[200] flex justify-center items-start pt-16 px-4 pb-4 overflow-hidden" 
                     x-cloak>
                    <!-- Overlay -->
                    <div x-show="isDetailOpen" 
                    ...
                    <div x-show="isDetailOpen" 
                         ...
                         class="bg-white rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)] w-full max-w-2xl flex flex-col max-h-[85vh] relative overflow-hidden border border-white/20 ring-1 ring-black/5">
                        
                        <!-- Premium Header -->
                        <div class="px-8 py-7 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-between flex-shrink-0 relative">
                            ...
                            <div class="flex items-center gap-5 relative z-10">
                                <div class="w-14 h-14 bg-gradient-to-tr from-emerald-500/20 to-emerald-400/5 backdrop-blur-xl rounded-[1.25rem] flex items-center justify-center border border-white/10 shadow-2xl group">
                                    <svg class="w-7 h-7 text-emerald-400 transform group-hover:scale-110 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                    </svg>
                                </div>
                                <div>
                                    <h3 class="text-xl font-black text-white tracking-tight leading-tight">Detail Potensi Lahan</h3>
                                    <div class="flex items-center gap-3 mt-2">
                                        <span class="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-[10px] font-black rounded-full border border-emerald-500/20 tracking-wider uppercase">ID: <span x-text="activeDetailData?.id_lahan"></span></span>
                                        <span class="px-3 py-1 bg-white/5 text-slate-400 text-[10px] font-black rounded-full border border-white/5 tracking-wider uppercase" x-text="activeDetailData?.id_tingkat"></span>
                                    </div>
                                </div>
                            </div>
                            <button @click="isDetailOpen = false" class="relative z-10 text-white/40 hover:text-white bg-white/5 hover:bg-rose-500/20 p-3.5 rounded-2xl transition-all duration-500 border border-white/5 hover:border-rose-500/30 active:scale-90 group">
                                <svg class="w-5 h-5 transform group-hover:rotate-90 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="px-8 pt-6 pb-2 bg-slate-50/50 backdrop-blur-md flex-shrink-0 border-b border-slate-100">
                            <div class="flex p-1.5 bg-slate-200/40 rounded-[1.5rem] gap-1.5 border border-slate-200/50">
                                <template x-for="tab in [{id:'personel', label:'👤 Personel'}, {id:'teknis', label:'📊 Teknis'}, {id:'lokasi', label:'📍 Lokasi'}, {id:'validasi', label:'✅ Validasi'}]">
                                    <button @click="activeTab = tab.id" 
                                            :class="activeTab === tab.id ? 'bg-white text-emerald-600 shadow-[0_8px_16px_-4px_rgba(0,0,0,0.1)] border-emerald-100/50' : 'text-slate-500 hover:text-slate-700 hover:bg-white/60'"
                                            class="flex-1 py-3.5 text-[11px] font-black uppercase rounded-2xl transition-all duration-500 border border-transparent outline-none"
                                            x-text="tab.label">
                                    </button>
                                </template>
                            </div>
                        ...
                        <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar bg-slate-50/50 px-8 pb-8 pt-6">
                            
                            <!-- Tab: Personel -->
                            <div x-show="activeTab === 'personel'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                                <div class="grid grid-cols-2 gap-5">
                                    <div class="p-6 bg-white border border-slate-200/60 shadow-sm rounded-[2rem] hover:border-emerald-200 transition-all duration-500 group">
                                        <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2 group-hover:text-emerald-500 transition-colors">Kepolisian Resor</p>
                                        <p class="text-[15px] font-bold text-slate-800" x-text="activeDetailData?.nama_polres"></p>
                                    </div>
                                    <div class="p-6 bg-white border border-slate-200/60 shadow-sm rounded-[2rem] hover:border-emerald-200 transition-all duration-500 group">
                                        <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2 group-hover:text-emerald-500 transition-colors">Kepolisian Sektor</p>
                                        <p class="text-[15px] font-bold text-slate-800" x-text="activeDetailData?.nama_polsek || '-'"></p>
                                    </div>
                                </div>

                                <!-- Polisi Penggerak Card -->
                                <div class="p-7 bg-emerald-50/50 border border-emerald-100 rounded-[2.5rem] relative overflow-hidden group">
                                    <div class="absolute -right-10 -top-10 w-40 h-40 bg-emerald-100/40 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000 pointer-events-none"></div>
                                    <p class="text-[11px] font-black uppercase tracking-[0.2em] text-emerald-600 flex items-center gap-2 mb-4 relative z-10">
                                        <span class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                                        🚔 Polisi Penggerak
                                    </p>
                                    <div class="flex items-center justify-between relative z-10">
                                        <p class="text-xl font-black text-slate-900 uppercase tracking-tight" x-text="activeDetailData?.cp_polisi"></p>
                                        <a :href="'https://wa.me/' + (activeDetailData?.no_cp_polisi?.replace(/\D/g,'').startsWith('0') ? '62' + activeDetailData?.no_cp_polisi?.replace(/\D/g,'').substring(1) : activeDetailData?.no_cp_polisi?.replace(/\D/g,''))" 
                                           target="_blank" 
                                           class="flex items-center gap-3 px-6 py-3 bg-emerald-500 text-white rounded-2xl hover:bg-emerald-600 transition-all duration-500 shadow-[0_12px_24px_-8px_rgba(16,185,129,0.4)] active:scale-95 group/btn">
                                            <span class="text-xs font-black uppercase tracking-widest" x-text="activeDetailData?.no_cp_polisi"></span>
                                            <svg class="w-5 h-5 transform group-hover/btn:translate-x-1 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.77-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86s.274.072.376-.043c.101-.116.433-.506.549-.68.116-.173.231-.145.39-.087s1.011.477 1.184.564.289.13.332.202c.045.072.045.419-.099.824z" /></svg>
                                        </a>
                                    </div>
                                </div>

                                <!-- Penanggung Jawab Card -->
                                <div class="p-7 bg-blue-50/50 border border-blue-100 rounded-[2.5rem] relative overflow-hidden group">
                                    <div class="absolute -right-10 -top-10 w-40 h-40 bg-blue-100/40 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000 pointer-events-none"></div>
                                    <p class="text-[11px] font-black uppercase tracking-[0.2em] text-blue-600 flex items-center gap-2 mb-4 relative z-10">
                                        <span class="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></span>
                                        👥 Penanggung Jawab
                                    </p>
                                    <div class="flex items-center justify-between relative z-10">
                                        <p class="text-xl font-black text-slate-900 uppercase tracking-tight" x-text="activeDetailData?.cp_lahan"></p>
                                        <a :href="'https://wa.me/' + (activeDetailData?.no_cp_lahan?.replace(/\D/g,'').startsWith('0') ? '62' + activeDetailData?.no_cp_lahan?.replace(/\D/g,'').substring(1) : activeDetailData?.no_cp_lahan?.replace(/\D/g,''))" 
                                           target="_blank" 
                                           class="flex items-center gap-3 px-6 py-3 bg-blue-500 text-white rounded-2xl hover:bg-blue-600 transition-all duration-500 shadow-[0_12px_24px_-8px_rgba(59,130,246,0.4)] active:scale-95 group/btn">
                                            <span class="text-xs font-black uppercase tracking-widest" x-text="activeDetailData?.no_cp_lahan"></span>
                                            <svg class="w-5 h-5 transform group-hover/btn:translate-x-1 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.77-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86s.274.072.376-.043c.101-.116.433-.506.549-.68.116-.173.231-.145.39-.087s1.011.477 1.184.564.289.13.332.202c.045.072.045.419-.099.824z" /></svg>
                                        </a>
                                    </div>
                                </div>

                                <div class="p-6 bg-white border border-slate-200/60 shadow-sm rounded-[2rem]">
                                    <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2">Keterangan Lain</p>
                                    <p class="text-[14px] font-bold text-slate-700 leading-relaxed italic" x-text="activeDetailData?.ket_polisi || 'Tidak ada keterangan tambahan.'"></p>
                                </div>
                            </div>

                            <!-- Tab: Teknis -->
                            <div x-show="activeTab === 'teknis'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                                <div class="grid grid-cols-2 gap-5">
                                    <div class="p-6 bg-white border border-slate-200/60 shadow-sm rounded-[2rem] group">
                                        <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2 group-hover:text-blue-500 transition-colors tracking-widest">Jenis Lahan</p>
                                        <p class="text-[15px] font-black text-slate-800 uppercase" x-text="activeDetailData ? {1:'PRODUKTIF (POKTAN BINAAN POLRI)', 2:'HUTAN (PERHUTANAN SOSIAL)', 3:'LUAS BAKU SAWAH (LBS)', 4:'PESANTREN', 5:'MILIK POLRI', 6:'PRODUKTIF (MASYARAKAT BINAAN POLRI)', 7:'PRODUKTIF (TUMPANG SARI)', 8:'HUTAN (PERHUTANI/INHUTANI)', 9:'LAHAN LAINNYA'}[activeDetailData.id_jenis_lahan] : '-'"></p>
                                    </div>
                                    <div class="p-6 bg-white border border-slate-200/60 shadow-sm rounded-[2rem] group">
                                        <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2 group-hover:text-emerald-500 transition-colors tracking-widest">Komoditi Utama</p>
                                        <p class="text-[15px] font-black text-slate-800 uppercase" x-text="activeDetailData?.nama_komoditi"></p>
                                    </div>
                                </div>
                                <div class="grid grid-cols-3 gap-5">
                                    <div class="p-6 bg-emerald-50/50 border border-emerald-100 rounded-[2.25rem] text-center shadow-sm group hover:scale-[1.02] transition-transform duration-500">
                                        <p class="text-[9px] font-black uppercase tracking-widest text-emerald-600 mb-3">Luas Lahan</p>
                                        <p class="text-4xl font-black text-emerald-700 tracking-tighter" x-text="parseFloat(activeDetailData?.luas_lahan || 0).toFixed(2)"></p>
                                        <p class="text-[10px] text-emerald-600 font-black mt-2 tracking-widest uppercase opacity-60">Hektar (HA)</p>
                                    </div>
                                    <div class="p-6 bg-white border border-slate-200/60 rounded-[2.25rem] text-center shadow-sm group hover:scale-[1.02] transition-transform duration-500">
                                        <p class="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Jml. Poktan</p>
                                        <p class="text-4xl font-black text-slate-800 tracking-tighter" x-text="activeDetailData?.poktan"></p>
                                        <p class="text-[10px] text-slate-400 font-black mt-2 tracking-widest uppercase opacity-60">Kelompok</p>
                                    </div>
                                    <div class="p-6 bg-white border border-slate-200/60 rounded-[2.25rem] text-center shadow-sm group hover:scale-[1.02] transition-transform duration-500">
                                        <p class="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Jml. Petani</p>
                                        <p class="text-4xl font-black text-slate-800 tracking-tighter" x-text="activeDetailData?.jml_petani"></p>
                                        <p class="text-[10px] text-slate-400 font-black mt-2 tracking-widest uppercase opacity-60">Orang</p>
                                    </div>
                                </div>
                            </div>

                            <!-- Tab: Lokasi -->
                            <div x-show="activeTab === 'lokasi'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                                <div class="p-7 bg-white border border-slate-200/60 shadow-sm rounded-[2.25rem] hover:border-sky-200 transition-all duration-500 group">
                                    <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-3 group-hover:text-sky-500 transition-colors">Wilayah Administrasi</p>
                                    <p class="text-[15px] font-bold text-slate-800 leading-relaxed" x-text="activeDetailData ? (activeDetailData.id_wilayah + ' - ' + [activeDetailData.kab_nama, activeDetailData.kec_nama, activeDetailData.desa_nama].filter(Boolean).join(' → ')) : '-'"></p>
                                </div>
                                <div class="p-7 bg-white border border-slate-200/60 shadow-sm rounded-[2.25rem] hover:border-sky-200 transition-all duration-500 group">
                                    <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-3 group-hover:text-sky-500 transition-colors">Alamat & Keterangan Lahan</p>
                                    <p class="text-[15px] font-bold text-slate-700 leading-relaxed mb-4" x-text="activeDetailData?.alamat_lahan"></p>
                                    <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 italic text-[13px] text-slate-500" x-text="activeDetailData?.keterangan_lahan || 'Tidak ada deskripsi lahan.'"></div>
                                </div>
                                <div class="grid grid-cols-2 gap-5">
                                    <div class="p-6 bg-white border border-slate-200/60 shadow-sm rounded-[2rem] group">
                                        <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2 group-hover:text-sky-500 transition-colors tracking-widest">Latitude</p>
                                        <p class="text-[15px] font-mono font-black text-sky-600" x-text="activeDetailData?.latitude"></p>
                                    </div>
                                    <div class="p-6 bg-white border border-slate-200/60 shadow-sm rounded-[2rem] group">
                                        <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2 group-hover:text-sky-500 transition-colors tracking-widest">Longitude</p>
                                        <p class="text-[15px] font-mono font-black text-sky-600" x-text="activeDetailData?.longitude"></p>
                                    </div>
                                </div>
                                <a :href="'https://www.google.com/maps?q=' + activeDetailData?.latitude + ',' + activeDetailData?.longitude" 
                                   target="_blank" 
                                   class="flex items-center justify-center gap-3 p-5 bg-sky-50 text-sky-600 border border-sky-100 rounded-[2rem] text-[12px] font-black uppercase tracking-widest hover:bg-sky-600 hover:text-white hover:shadow-[0_12px_24px_-8px_rgba(2,132,199,0.4)] transition-all duration-500 active:scale-[0.98]">
                                    <svg class="w-5 h-5 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /></svg>
                                    Buka Navigasi Google Maps
                                </a>
                                <div x-show="activeDetailData?.dokumentasi_lahan" class="pt-2">
                                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 ml-2">Dokumentasi Visual</p>
                                    <div class="relative group overflow-hidden rounded-[2.5rem] border-4 border-white shadow-2xl">
                                        <img :src="'/' + activeDetailData?.dokumentasi_lahan" alt="Dokumentasi Lahan" class="w-full object-cover max-h-72 transform group-hover:scale-110 transition-transform duration-[2000ms]">
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex items-end p-8">
                                            <p class="text-white text-xs font-black uppercase tracking-[0.2em]">Dokumentasi Lapangan</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Tab: Validasi -->
                            <div x-show="activeTab === 'validasi'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                                <div class="p-8 bg-white border border-slate-200/60 shadow-sm rounded-[2.5rem] group hover:border-slate-300 transition-all duration-500">
                                    <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-4 group-hover:text-slate-600 transition-colors">Data Entri / Terakhir Diubah</p>
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="text-lg font-black text-slate-800 uppercase tracking-tight" x-text="activeDetailData?.edit_oleh"></p>
                                            <p class="text-xs font-bold text-slate-400 mt-1 flex items-center gap-1.5"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> <span x-text="activeDetailData?.tgl_edit"></span></p>
                                        </div>
                                    </div>
                                </div>

                                <div :class="activeDetailData?.valid_oleh ? 'bg-emerald-50/50 border-emerald-100' : 'bg-amber-50/50 border-amber-100'" 
                                     class="p-8 border rounded-[2.5rem] shadow-sm relative overflow-hidden transition-all duration-700">
                                    <div x-show="activeDetailData?.valid_oleh" class="absolute -right-6 -top-6 w-32 h-32 bg-emerald-500/10 rounded-full blur-2xl"></div>
                                    <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-4">Status Validasi System</p>
                                    <div class="flex items-center gap-4 relative z-10">
                                        <div :class="activeDetailData?.valid_oleh ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-white'" class="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg">
                                            <template x-if="activeDetailData?.valid_oleh">
                                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /></svg>
                                            </template>
                                            <template x-if="!activeDetailData?.valid_oleh">
                                                <svg class="w-8 h-8 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                            </template>
                                        </div>
                                        <div>
                                            <p :class="activeDetailData?.valid_oleh ? 'text-emerald-700' : 'text-amber-700'" class="text-xl font-black uppercase tracking-tight" x-text="activeDetailData?.valid_oleh ? activeDetailData?.valid_oleh : 'Menunggu Validasi Admin'"></p>
                                            <p x-show="activeDetailData?.valid_oleh" class="text-xs font-bold text-emerald-600/60 mt-1 flex items-center gap-1.5"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> <span x-text="activeDetailData?.tgl_valid"></span></p>
                                            <p x-show="!activeDetailData?.valid_oleh" class="text-xs font-bold text-amber-600/60 mt-1">Harap hubungi Admin Polda untuk verifikasi data.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Premium Footer -->
                        <div class="p-6 bg-white border-t border-slate-100 flex justify-end flex-shrink-0">
                            <button @click="isDetailOpen = false" 
                                    class="px-8 py-3.5 bg-slate-900 text-white text-[11px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-slate-800 transition-all duration-500 shadow-[0_12px_24px_-8px_rgba(15,23,42,0.4)] active:scale-95">
                                Tutup Informasi
                            </button>
                        </div>
                    </div>
            ...
            <div x-show="isEditOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/30" x-cloak>
                <div @click.outside="isEditOpen = false" x-show="isEditOpen" x-transition.opacity.duration.300ms class="bg-white rounded-[2rem] shadow-2xl w-full max-w-xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh]">
                    ...
                    <form :action="`/operator/kelola-lahan/potensi/update/${activeData?.id_lahan}`" method="POST" class="flex-1 overflow-y-auto custom-scrollbar">
                        @csrf @method('PUT')
            ...
            <!-- 3. DELETE MODAL -->
            <div x-show="isDeleteOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/30" x-cloak>
        ...
        <script>
                ...
                return {
                    ...
                    select(anggota) {
                        ...
                        if (type === 'penggerak') {
                            ...
                            // Dispatch to parent
                            this.$dispatch('set-penggerak', {
                                nama: anggota.nama_anggota,
                                hp: anggota.no_telp_anggota || ''
                            });
                        } else {
                            this.$root._x_dataStack?.forEach(d => {
                                if (d.formData) {
            ...
            function potensiLahanManager() {
                return {
                    ...
                    submitFilters() {
                        const url = new URL(window.location.href);
                        if (this.selectedResor) url.searchParams.set('resor', this.selectedResor);
                        else url.searchParams.delete('resor');
                        if (this.selectedSektor) url.searchParams.set('sektor', this.selectedSektor);
                        else url.searchParams.delete('sektor');
                        if (this.selectedJenis) url.searchParams.set('jenis', this.selectedJenis);
                        else url.searchParams.delete('jenis');
                        if (this.selectedValidasi) url.searchParams.set('validasi', this.selectedValidasi);
                        else url.searchParams.delete('validasi');
                    ...
                    getCurrentLocation() {
                        if (navigator.geolocation) {
                            navigator.geolocation.getCurrentPosition(
                                ...
                                (error) => {
                                $notify('error', 'GPS Gagal', 'Gagal mendapatkan lokasi GPS: ' + error.message);
                                }, {
                                    enableHighAccuracy: true
                                }
                            );
                        } else {
                            $notify('warning', 'Browser Tidak Mendukung', 'Browser Anda tidak mendukung fitur Geolocation.');
                        }
                    },
                    ...
                    async saveData() {
                        ...
                        for (const field of requiredFields) {
                            if (!this.formData[field] && this.formData[field] !== 0) {
                                $notify('warning', 'Form Belum Lengkap', 'Mohon lengkapi semua data formulir yang tersedia di setiap tahapan!');
                                return;
                            }
                        }

                        if (!this.isEdit && !this.formData.photoFile) {
                            $notify('warning', 'Foto Diperlukan', 'Mohon unggah Foto Dokumentasi Lahan terlebih dahulu.');
                            return;
                        }
                        ...
                        try {
                            ...
                            if (response.ok) {
                                $notify('success', 'Data Berhasil Disimpan!', 'Potensi lahan telah berhasil dicatat dalam sistem.');
                                this.closeModal();
                                setTimeout(() => window.location.reload(), 1500);
                            } else {
                    ...
                    $confirm({
                        ...
                        message: 'Data ini akan dihapus dari sistem secara permanen.',
                        confirmText: 'Ya, Hapus'
                    }).then(ok => {
                        if (ok) console.log('Menghapus ID:', id);
                    });
                    },
                    init() {
